...
struct ReleasedRecordingsView: View {
    ...
    // Set only by the -MOCK_RELEASED_ERROR seam, so the offline UITest can prove
    // "Try again" actually leaves the error state.
    @State private var mockErrorCleared = false
    ...
    private var errorText: String? {
        if TestConfig.isMockReleasedError {
            return mockErrorCleared ? nil : ReleasedRecordingsService.sessionExpiredMessage
        }
        ...
        NavigationStack {
            Group {
                if isLoading {
                    ...
                    // until the app was force-quit (#1070).
    ...
    private func empty(_ icon: String, _ title: String, _ subtitle: String,
        ...
        // actionable variant is addressed via the button's "empty-action" id.
        if action == nil {
